...
def make_returns_report(event, context):
    ...
    df_return_rates_grouped = \
        df_return_rates.groupby('billing_company')[
            'orders', 'service'].sum()  # Calculate the orders and returns by distributor
    ...
    df_return_rates_grouped \
        = df_return_rates_grouped[
        df_return_rates_grouped['return_rate'] != 0]  # Remove distributors which never returned defectives
...
def plot_pivot_orders(event, context):
    ...
    if log_path:
        ...
        # logging
        table_format = 'psql'
        logger.info(tabulate.tabulate(headers, headers=headers.columns, tablefmt=table_format))
        logger.info(tabulate.tabulate(lines, headers=lines.columns, tablefmt=table_format))
        logger.info(tabulate.tabulate(return_rates, headers=return_rates.columns, tablefmt=table_format))
...
def plot_dashboard(event, context):
    ...
    # Plot figures
    args = [tseries_ds,
            # fillna(0) necessary to continuity of the line in case of gaps
            unit_price_orderline_df.fillna(0),
            channels_ds,
            time_start,
            window_roll,
            figsize,
...
def plot_distributors(df,
    ...
    # setting to show the xticklabels properly
